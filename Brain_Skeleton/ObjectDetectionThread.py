...
class ObjectDetectionThread(Thread):
    def __init__(self, inP_img, outP_obj, brain):
        ...
        self.stabilizer = ObjectStabilizer(5, 0.1)
    ...
    def perform_object_detection(self, image):
        ...
        for idx in range(len(ids)):
            ...
            if score < config.DETECTION_SCORE_THRESHOLD*100:
                continue
                                                                    # w                # h
            if color and label_text and accept_box(x1, x2, y1, y2, image.shape[1], image.shape[0]) :# and accept_box(boxes, box, 5.0):
    ...
    def init_models(self):
        ...
        if config.RUN_MODE == "TFLITE":
            ...
            self.object_detector_tflite = TFLiteModel("models/mobilenet_320_aug_extended_edgetpu.tflite",
